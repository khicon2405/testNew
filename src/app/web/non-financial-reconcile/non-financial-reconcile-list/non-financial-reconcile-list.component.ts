...
@Component({
  selector: "ite-non-financial-reconcile-list",
  templateUrl: "./non-financial-reconcile-list.component.html",
  styleUrls: ["./non-financial-reconcile-list.component.scss"],
})
export class NonFinancialReconcileListComponent implements OnInit {
  public dataTable = [];
  ...
  constructor(
    ...
    this.reconcileCode = RECONCILE.RECONCILE_CODE_NON_FINANCIAL_TRANS;
    this.transChannels = [];
  ...
  getDataTable(request) {
    ...
    this.dataTable = [];
    this.bambooService
      .getNonFinancialReconcileResult(request)
      .subscribe((data) => {
  ...
  exportFileExcel() {
    let request = this.getFromData();
    if (request != null) {
      this.bambooService.exportNonFinancialReconcileResult(request);
    }
  }

  viewDetail(item) {
    this.router.navigate([`admin/non-financial-reconcile/details/${item}`]);
  }
}
