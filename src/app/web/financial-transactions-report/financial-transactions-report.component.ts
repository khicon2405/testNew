...
@Component({
  selector: "ite-financial-transactions-report",
  templateUrl: "./financial-transactions-report.component.html",
  styleUrls: ["./financial-transactions-report.component.scss"],
})
export class FinancialTransactionsReportComponent implements OnInit {
  public loading = true;
  ...
  getDataTable(request) {
    ...
    this.bambooService
      .getReportWalletReconcileDetail(request)
      .subscribe((data) => {
  ...
  exportFileExcel() {
    let request = this.getFromData();
    if (request != null) {
      this.bambooService.exportReportWalletReconcileDetail(request);
    }
  }
