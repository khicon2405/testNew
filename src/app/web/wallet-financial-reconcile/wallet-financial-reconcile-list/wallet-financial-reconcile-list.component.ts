...
@Component({
  selector: "ite-wallet-financial-reconcile-list",
  templateUrl: "./wallet-financial-reconcile-list.component.html",
  styleUrls: ["./wallet-financial-reconcile-list.component.scss"],
})
export class WalletFinancialReconcileListComponent implements OnInit {
  public dataTable = [];
  ...
  constructor(
    ...
    this.reconcileCode = RECONCILE.RECONCILE_CODE_FINANCIAL_TRANS;
    this.transChannels = [];
    this.transTypeLists = RECONCILE.WALLET_TRANS_TYPE;
    this.paymentMethods = RECONCILE.NON_FINANCIAL_PAYMENT_METHODS;
  }
  ...
  getFromData() {
    ...
    //   this.searchRequest.listChannel = form.listTransChannel;
    // }
  ...
  getDataTable(request) {
    this.loading = true;
    this.dataTable = [];
    this.bambooService.getWalletReconcileResult(request).subscribe((data) => {
  ...
  exportFileExcel() {
    let request = this.getFromData();
    if (request != null) {
      this.bambooService.exportWalletReconcileResult(request);
    }
  }
  viewDetail(item) {
    this.router.navigate([`admin/wallet-financial-reconcile/details/${item}`]);
  }
