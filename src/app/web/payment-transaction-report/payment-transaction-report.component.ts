...
@Component({
  selector: "ite-payment-transaction-report",
  templateUrl: "./payment-transaction-report.component.html",
  styleUrls: ["./payment-transaction-report.component.scss"],
})
export class PaymentTransactionReportComponent implements OnInit {
  ...
  getDataTable(request) {
    ...
    this.bambooService
      .getReportPaygateReconcileDetail(request)
      .subscribe((data) => {
  ...
  exportFileExcel() {
    let request = this.getFromData();
    if (request != null) {
      this.bambooService.exportReportPaygateReconcileDetail(request);
    }
  }
